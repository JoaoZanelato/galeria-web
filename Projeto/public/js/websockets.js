document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof userId !== 'undefined' && userId) {
        ...
        // Envia o ID do usuário para o servidor se juntar à sua sala privada
        socket.on('connect', () => {
            socket.emit('join', userId);
        });

        // Ouve por novas solicitações de amizade
        socket.on('nova_solicitacao', (data) => {
            alert(data.message);
            // Opcional: Adicionar uma notificação mais sofisticada no DOM
        });
        
        // Ouve por solicitações aceitas
        ...
        // Ouve por novos compartilhamentos
